...
const NavBarNew: FC<NavBarProps> = ({ variant = 'default' }) => {
    ...
    <nav className={isAuth ? 'auth-navbar' : 'navbar'}>
      <div className={isAuth ? 'auth-nav-container' : 'nav-container'}>
        ...
        <div className={isAuth ? 'auth-nav-buttons' : 'nav-buttons'}>
          <Link to="/signup" className={isAuth ? 'auth-btn-join' : 'btn-join'}>Únete</Link>
          <Link to="/login" className={isAuth ? 'auth-btn-login' : 'btn-login'}>Iniciar sesión</Link>
        </div>
      </div>
    </nav>
  );
